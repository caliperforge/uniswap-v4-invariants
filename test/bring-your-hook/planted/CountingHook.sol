...
contract CountingHook {
    ...
    function afterSwap(address, PoolKey calldata, SwapParams calldata params, BalanceDelta, bytes calldata)
        external
        returns (bytes4, int128)
    {
        // SEEDED SPECIFICATION VIOLATION (single hunk vs the clean
        // fixture at test/base/hooks/CountingHook.sol): the tally
        // skips oneForZero swaps, violating the swap-count property
        // the example suite declares.
        if (params.zeroForOne) afterSwapCount++;
