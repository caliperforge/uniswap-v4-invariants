...
contract InvariantRouter is IUnlockCallback {
    ...
    /// Native-pair mode: `V4TestBase` pre-funds the router via `vm.deal` so
    /// buy-side settle draws from the router's own balance rather than
    /// plumbing msg.value through every call. Sell-side `take` may also
    /// route native ETH to callers that happen to be this router in
    /// hop-through patterns.
    ...
    function _resolve(Currency currency, address payer, int128 amount) internal {
        ...
        if (amount < 0) {
            ...
            if (Currency.unwrap(currency) == address(0)) {
                // Native settle: PoolManager._settle() branches on
                // "syncedCurrency is native or unset" and applies msg.value
                // to the native delta; no sync needed. The router pre-funds
                // itself via receive() so this draws from its own balance.
                manager.settle{value: owed}();
            } else {
                manager.sync(currency);
                // The transferFrom return is not checked because on revert
                // the whole unlock unwinds and any non-standard non-reverting
                // failure leaves the delta unsettled, which the PoolManager
                // surfaces as CurrencyNotSettled.
